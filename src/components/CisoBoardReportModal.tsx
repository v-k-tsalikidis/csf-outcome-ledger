...
export const CisoBoardReportModal: React.FC<CisoBoardReportModalProps> = ({
  ...
  const supportedCount = outcomes.filter(
    (o) => (decisions[o.id]?.status || 'UNSUPPORTED') === 'SUPPORTED'
  ).length;
  const gapCount = outcomes.filter(
    (o) => (decisions[o.id]?.status || 'UNSUPPORTED') === 'UNSUPPORTED'
  ).length;
  const staleCount = outcomes.filter(
    (o) => (decisions[o.id]?.status || 'UNSUPPORTED') === 'STALE'
  ).length;
        ...
        <div className="space-y-6 font-sans">
          <div className="border-b border-zinc-200 pb-4">
            ...
            <p className="text-xs text-zinc-500 mt-1">
              Framework Alignment: NIST CSF 2.0 • SP 800-53 Rev. 5 • EU DORA (Art. 5-14)
              • NIS2 (Art. 21)
            </p>
            <div className="text-[11px] font-mono text-zinc-400 mt-2">
              Generated Date: {new Date().toLocaleDateString()} | Auditor: Basil
              Tsalikidis (InfoSec Officer)
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-zinc-50 border border-zinc-200 rounded-md p-3">
              ...
              <div className="text-lg font-bold text-zinc-900 mt-0.5">
                {threatIndex.catalogueSize !== null
                  ? threatIndex.catalogueSize.toLocaleString()
                  : 'n/a'}
              </div>
              <div className="text-[10px] text-zinc-400 font-mono">
                {threatIndex.source === 'live'
                  ? 'entries, retrieved live'
                  : 'live feed unavailable'}
              </div>
            </div>

            <div className="bg-emerald-50/60 border border-emerald-200/60 rounded-md p-3">
              <div className="text-[10px] text-emerald-800 font-medium uppercase">
                Supported Outcomes
              </div>
            ...
            <div className="bg-amber-50/60 border border-amber-200/60 rounded-md p-3">
              <div className="text-[10px] text-amber-800 font-medium uppercase">
                Stale / Review Due
              </div>
            ...
            <div className="bg-rose-50/60 border border-rose-200/60 rounded-md p-3">
              <div className="text-[10px] text-rose-800 font-medium uppercase">
                Critical Gaps
              </div>
          ...
          <div>
            ...
            <p className="text-xs text-zinc-700 leading-relaxed bg-zinc-50 border border-zinc-200/80 rounded-md p-3">
              This decision ledger documents the organizational security posture against
              NIST CSF 2.0, NIST SP 800-53 Rev. 5, and EU DORA/NIS2 mandates.
              Cryptographic SHA-256 evidence hashing has been established for all
              approved controls, ensuring immutable audit provenance with zero cloud
              data exposure.
            </p>
          </div>
                    ...
                    return (
                      <tr key={o.id}>
                        ...
                        <td className="px-3 py-2 font-mono text-[10px] text-zinc-500 truncate max-w-[140px]">
                          {dec?.evidence?.referenceHash
                            ? dec.evidence.referenceHash.substring(0, 12) + '...'
                            : 'None'}
                        </td>
                      </tr>
                    );
